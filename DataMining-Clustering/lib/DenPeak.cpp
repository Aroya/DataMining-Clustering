...
void DenPeak::input(double ** Data, const int & Rows, const int & Columns){
	if (data) {
...
void DenPeak::setMaximumDistance(const double&t) {
	distance_maximum = t;
}

void DenPeak::writeFile(const string & fileName)
{
	ofstream fout;
	fout.open(fileName);
	if (fout.is_open()) {
